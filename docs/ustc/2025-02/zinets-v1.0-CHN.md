...
# 汉字的计算网络分析
《说文解字》是东汉许慎编撰的一部奠基性著作，代表了对汉字结构和词源进行系统分析的首次尝试 1,2,3。许慎识别出540个部首（部首 , bùshǒu ），并运用其“六书”（六书 , liùshū ）理论解释汉字的构成，将汉字按照其组成分类为象形字（象形 ）、指事字（指事 ）、会意字（会意 ）、形声字（形声 ），以及两个较少见的类别（转注 和假借 ）。尽管具有开创性，《说文解字》的分析主要基于小篆（小篆 ），并不能完全反映许多现代汉字的形式，而其540个部首虽然有价值，但并不总是最小且不可再分的组成部分。

后来的《字汇》和《康熙字典》对这一系统进行了改进和简化，最终确定了214个康熙部首 4，这些部首成为标准的索引系统。这些部首主要基于共享的视觉成分，通常与意义相关（形旁，形旁 , xíngpáng ），为字典查询提供了方法。虽然许多部首也暗示了发音（声旁，声旁 , shēngpáng ），但这种提示并不总是可靠。尽管康熙系统被广泛使用，但它存在局限性：分类模糊、形旁和声旁的作用不一致，以及一些因复杂性或低频出现而并非总是最能揭示字形结构的部首。

在继承许慎和康熙系统的基础上，作者开发了一个名为“ZiNets”的网络应用工具，并结合计算网络结构，对6190个汉字 [5]（包括3910个HSK常用汉字 [6]）进行了分解。通过设定频率阈值，我们识别出了作为高频出现的组成部分的“元字”。

为了系统地分析汉字结构，我们引入了一种新的空间分解模型——“字矩阵”（Zi-Matrix ）。该模型将每个汉字表示为一个最多包含11个不同位置组件的矩阵。这些位置定义为：上（上 ）、下（下 ）、左（左 ）、右（右 ）、中（中 ）、左上（左上 ）、右上（右上 ）、左下（左下 ）、右下（右下 ）、内中（中内 ）和外中（中外 ）。矩阵中的每个位置可以由特定的字组件（部首、笔画或更复杂的子组件）占据，也可以为空。这种11组件的矩阵提供了一种一致且结构化的表示方式，用于描述任何汉字的组成部分之间的空间关系，无论其复杂程度如何。

字矩阵空间分解模型的示意图，用于汉字网络分析。 {width="85%"}
层次分解树图展示了从左到右逐步将汉字分解为其组成部分的过程。此可视化展示了复杂汉字典型的多层次结构组成。 {width="85%"}

每个汉字的分解过程采用手动的层次化方法完成。该过程涉及一系列分解步骤。首先，将汉字拆分为其主要组成部分，并将其分配到矩阵中的适当位置。如果某个组成部分本身较为复杂，则进一步递归分解为其子部分，并将其分配到表示该组件的子矩阵中的相应位置。这种层次化分解持续进行，直到识别出最基本的组成部分——那些无法合理进一步拆分的部分，如图2所示。这些基本组成部分成为“元字”集合的候选者。这种手动的层次化方法确保了对汉字结构的一致分析，同时利用语言学专家的知识指导分解过程。

# # 元字分析
通过使用字矩阵方法对6190个汉字进行手动分解，我们识别出422个独特且不可再分的组成部分，称为“元字”（**元字**, *yuánzì*）。这些组成部分代表了在我们的层次化分解过程中浮现的基本构建单元。表1按笔画数列出了这些元字，并明确区分了传统的康熙部首与新识别的组成部分。

## 按笔画数与来源分类的元字（**元字**）- 表1 {#按笔画数与来源分类的元字-表1}
...
识别出的元字集合包含245个源自传统康熙部首系统的组成部分，并新增了177个额外的组件。这一扩展反映了我们在字符分解过程中采用的更精细的方法，其中一些传统上被归为一组的部首被视为独立的元素。这些新识别的组件虽然在标准字典中未被传统地视为独立单位，但在汉字构成中发挥了重要的语音和/或语义作用。例如，像“禺”和“乍”这样的组件，尽管在传统系统中未被归类为部首，但在复合字中表现出一致的语义贡献，我们将在案例研究中进一步探讨这一点。

我们的元字集合揭示了比传统康熙系统更为精细的汉字构成方式。它涵盖了三种主要类型的组件：熟悉的表意部首（如氵[水]、木[木]、日[日]、月[月]、心[心]、手[手]、口[口]）、基本结构元素（如一、丨、丿、丶、乙、口、凵、冂），以及频繁出现且兼具语音和语义信息的组件（如方、占、且、戋、乍、禺、尧、佥）。一些继承自康熙系统的组件，例如“鼓”（鼓），虽然具有历史意义，但可能需要重新评估其在现代实际应用中的适用性。这一全面的元字集合为理解和教授汉字构成提供了更为细致的基础，在简化学习过程的同时保持了语义完整性。
...
# 汉字的自然演化
在前文关于“可视化分类”和“字符构成模式”的章节中，我们观察到元字符如何反映人类对自然的认知，以及元字符如何像物理世界中的原子一样相互作用。在此，我们将探讨汉字可能如何自然演化。

历史背景
据传说，仓颉（Cangjie ）发明了汉字。他是公元前27世纪黄帝时期的一位史官。仓颉受到自然界万物的启发，包括动物的足迹、地貌和星辰。他相信，如果能够通过一幅画捕捉每件事物的独特特征，就可以创造一种书写系统。他对结绳记事法的局限性感到沮丧，而结绳记事是当时记录信息的主要方法。

作为一项里程碑式的事件，仓颉无疑为汉字书写系统的创建做出了重要贡献。然而，在人类语言的宏观发展中，汉字可能是代代相传、自然演化的结果，而非如普遍认为的那样是由仓颉凭空设计出来的神奇人工产物。

字符演化中的自然生长模式
正如自然系统从简单到复杂通过可预测的模式演化，汉字也展示了类似的有机发展过程。斐波那契数列（1, 1, 2, 3, 5, 8, 13, 21, 34, ...）为理解这一演化提供了一个优雅的框架。这并非严格的数学对应关系，而是作为一种隐喻，揭示了复杂性如何以系统化的方式从简单性中涌现。

自然界中斐波那契模式的图像——从向日葵种子排列到鹦鹉螺壳，从蕨类植物的叶子到螺旋星系——揭示了这一普遍的生长与组织原则。同样的原则可以帮助我们理解汉字如何从仓颉最初的灵感演变为今天的形态。

斐波那契螺旋在不同尺度上的自然表现：植物学（向日葵、多肉植物）、生物学（蜗牛壳、蕨类植物）、数学（黄金比例）、天文学（星系）示例。 {width="85%"}

受斐波那契数列等普遍自然生长模式的启发，本文作者借用这一序列来组织汉字的类似自然进展：从简单的象形文字到复杂的复合字符，从具体物体到抽象概念。这种自然进展不仅仅是一个组织框架，它还反映了人类认知本身如何从基本感知发展到复杂抽象的过程。正如斐波那契螺旋展示了复杂自然模式如何从简单的数学关系中涌现，我们的组织方式揭示了汉字书写如何从基本元素（元字）演化为日益复杂的表达形式。每一层级都引入新的基础字符，这些字符就像扩展的螺旋，成为更丰富语言表达的构建块。这种方法反映了自然界在简单基础上发展复杂系统的高效性。
...
总结
这九个层级展示了汉字如何从基本概念自然演化为复杂的人类表达方式，反映了自然系统中发现的复杂性梯度。
...
促进对文化与哲学维度的欣赏
支持与书写系统的创造性互动
本研究表明，真正的简化并非仅仅来自削减，而是源于揭示潜在的模式与原则。通过将汉字视为一个自然演化的体系，我们不仅在学习中获得了实际优势，还深化了对人类认知如何组织和传递知识的理解。
...
致敬
本研究谨献给已故的李政道教授（T.D. Lee），他开创性的努力为众多中国学生赴美学习和研究打开了大门，架起了东西方科学传统之间的桥梁。他的远见与支持使包括我在内的无数学者得以在全球科学对话中贡献力量。

本研究还献给作者的父母（龚永权，文国芳），他们在艰难困苦中培养了作者的求知欲。他们的牺牲与始终如一的支持将永远铭记于心。

致谢
本文是作者与一组人工智能助手共同协作的成果，这些助手包括来自Anthropic的Claude 3.3 10\]、来自Google的Gemini 2 \[11，以及来自阿里巴巴的Qwen2.5-Max 12。人类在软件开发、物理学和汉语领域的知识与洞察力，与人工智能的分析能力相结合，促成了本文中提出的全新视角与方法论的发展。这种协作展示了人类与人工智能在研究与学习中的合作潜力，尤其是在跨学科研究中，将传统知识与现代计算技术相融合的领域。
